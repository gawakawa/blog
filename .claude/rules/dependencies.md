...
After adding or updating dependencies, update the `fetchPnpmDeps` hash in `flakes/node-modules.nix`:

1. Run `nix flake check` — it will fail with a hash mismatch error
2. Copy the hash from the `got:` line in the error output
3. Replace the `hash` value in `flakes/node-modules.nix` with the new hash
